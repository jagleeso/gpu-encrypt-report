/* A modified version of opencl_aes.c from the original experiment:
 *
 * 1) Perform exhaustive error checks for an OpenCL call that returns an error code (or 
 * sets one via a pointer).
 *
 * 2) Call clGetDeviceInfo(device_id[0], CL_DEVICE_AVAILABLE, ...) to check if the device 
 * (i.e. GPU) is available before we proceed (refer to "Check if the device is available" 
 * comment).
 */
